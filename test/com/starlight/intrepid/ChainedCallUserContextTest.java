...
public class ChainedCallUserContextTest extends TestCase {
	...
	public void testChainedCall() throws Exception {
		...
		tail_instance.getLocalRegistry().bind( "test", tail_proxy_instance );
		...
		HeadInstance head_proxy_instance = new HeadInstance(
			...
			tail_instance.getLocalVMID() ).lookup( "test" ) );
		head_instance.getLocalRegistry().bind( "test", head_proxy_instance );
		...
		Runnable head_proxy =
			( Runnable ) client_instance.getRemoteRegistry( vmid ).lookup( "test" );
